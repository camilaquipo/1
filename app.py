...
#st.image('img/encabezado.png') #comando para la imagen 
...
with st.container(border=True):
    ...
    with col9: #creo el objeto 
        ...
        st.plotly_chart(fig, use_container_width=True) #que sea de todo el ancho del contenedor (en este caso una columna)

    with col10:
        df_mayores = df_agrupado.sort_values(by='ENERGÍA REACTIVA', ascending=False).head(5) #agrupacion y orden 
        fig = px.bar(
            df_mayores,
            x = 'MUNICIPIO',
            y = 'ENERGÍA REACTIVA',
    ...
    with col11:
        torta_activa = df_agrupado.sort_values(by='ENERGÍA ACTIVA',ascending=False).head(5)
        #torta_activa = df_agrupado.groupby('MUNICIPIO')['ENERGÍA ACTIVA'].sum().reset_index() #doble agrupado 
        #torta_activa = torta_activa.sort_values(by='ENERGÍA ACTIVA', ascending=False).head(5)
        fig1 = px.pie(
            torta_activa,
            names = 'MUNICIPIO', #ANTES DEPARTAMENTO 
            values = 'ENERGÍA ACTIVA', #####################################################################
            title = 'Paricipación Top 5 - Energía Activa',
            hole = 0.4  #tamaño del centro 
        )
